...
class ViaPointCalculationModeTest {
  ...
  void fromValue(SoftAssertions softly) {
    for (ViaPointCalculationMode expected : ViaPointCalculationMode.values()) {
      softly.assertThat(ViaPointCalculationMode.fromValue(expected.toString()))
          .isEqualTo(expected);
    }
    softly.assertThat(ViaPointCalculationMode.fromValue(null))
        .isEqualTo(ViaPointCalculationMode.FASTER_TIME);
  }
}